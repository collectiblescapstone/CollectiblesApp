        ...
        const fetchCardInfo = async () => {
            try {
                ...
                if (card?.grade === null || card?.gradeLevel === null) {
                    setCardCondition(cardConditionsMap[card?.condition ?? 'near-mint'] ?? 'Near Mint')
                } else {
                ...
                <Box height="50px">
                    <HStack>
                        <VStack align="start" width={"60%"} gap={1}>
                            <HStack><TbCards /> {capitalizeEachWord(cardInfo.variant)}</HStack>
                            <HStack><TbPlayCard /> {cardCondition}</HStack>
                        </VStack>
                        <HStack align="end" width={"40%"} justify="end" justifyContent="flex-end" gap={2} direction="row-reverse">
                            <FaEye size={24} />
                            {isForTrade && (
                                <AiOutlineSwap size={24} />
                            )}
                        </HStack>
                        <Button height="50px" width="50px" backgroundColor="brand.marigold" onClick={() => {
                            window.location.href = `/edit-card?cardId=${cardInfo.cardId}&entryId=${entryId}`
                        }}><FaPencilAlt size={24} fill="#003B49" /></Button>
                    </HStack>
                </Box>
            )}
        </Box>
