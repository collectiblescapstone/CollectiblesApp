...
const postProcessMaskServer = (
    ...
    try {
        ...
        const maskWeights = new Float32Array(
            NUM_FILTERED_RESULTS * MASK_CHANNELS
        )
        ...
        for (let i = 0; i < NUM_FILTERED_RESULTS; i++) {
            ...
            if (maskW > 0 && maskH > 0) {
                ...
                const maskRoi = maskMat.roi(
                    new cv.Rect(maskX, maskY, maskW, maskH)
                )
                ...
                const targetW = Math.min(
                    MODEL_INPUT_WIDTH - targetX,
                    Math.ceil(w)
                )
                const targetH = Math.min(
                    MODEL_INPUT_HEIGHT - targetY,
                    Math.ceil(h)
                )
